        ...
        >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0, flex: 1 }}>
                    ...
                    <span style={{ fontSize: fs(10), fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: TEXT_SECONDARY, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {typeLabel}
                    </span>
                    {isUrgent && (
                        <span style={{ fontSize: fs(9), fontWeight: 600, color: ACCENT_DANGER, background: 'rgba(220,38,38,0.15)', padding: '2px 6px', borderRadius: 4, flexShrink: 0 }}>Urgente</span>
                    )}
                    {card.metadata?.phase && (
                        <span style={{ fontSize: fs(9), fontWeight: 600, color: TEXT_MUTED, background: BORDER, padding: '2px 6px', borderRadius: 4, flexShrink: 0 }}>SPIN:{String(card.metadata.phase)}</span>
                    )}
                </div>
            ...
            <h4 style={{ fontWeight: 500, fontSize: fs(13), marginBottom: 2, color: TEXT, lineHeight: 1.25 }}>{card.title}</h4>
            <p style={{ fontSize: fs(12), color: TEXT_SECONDARY, lineHeight: 1.5 }}>
            ...
            {card.type === 'objection' && card.metadata?.objection && (
                <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 6, fontSize: fs(11), color: TEXT_SECONDARY, background: INPUT_BG, borderRadius: 4, padding: '4px 8px', border: `1px solid ${BORDER}` }}>
            ...
            {card.metadata?.suggested_response && (
                <div style={{ marginTop: 8, fontSize: fs(11), background: 'rgba(34,197,94,0.08)', borderRadius: 4, padding: '6px 8px', border: '1px solid rgba(34,197,94,0.25)' }}>
            ...
            {card.metadata?.suggested_question && (
                <div style={{ marginTop: 8, fontSize: fs(11), background: INPUT_BG, borderRadius: 4, padding: '6px 8px', border: `1px solid ${BORDER}` }}>
